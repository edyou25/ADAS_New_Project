...
def run_simulation(cfg, client):
    ...
    try:
        ...
        # Semantic Camera
        camera_sematic = init_camera(world, 'SemanticCamera', carla.Transform(carla.Location(x=0,y=0,z=2.4), 
                                     carla.Rotation(pitch=0, yaw=0, roll=0)), vehicle, IM_WIDTH, IM_HEIIGHT, 90 )
        ...
        lidar_queue = []
        lidar.listen(lambda image: process_lidar_image(image, IM_WIDTH, IM_HEIIGHT, lidar_queue, QUEUE_LEN))
        ...
        while True:   
            ...
            for walker_temp in walkers:
                if walker_temp.id != ego_id:
                    ...
                    if(distance) <  distance_threshold + buffer:
                        ...
                        other_vel_ms = random.uniform(2.5, 4.0)
                        other_traj = get_prediction(other_tf, other_vel_ms, other_steer, horizon, dt)
            ...
            p1.scatter(ego_tf.location.x, -ego_tf.location.y, c ='r')
            for  veh_temp in surrounding_vehicle:         
                location =  veh_temp.get_transform().location
                p1.scatter(location.x, -location.y, c ='b')
            for  walker_temp in surrounding_walker:         
                location =  walker_temp.get_transform().location
                p1.scatter(location.x, -location.y, c ='g')
            plt.axis('equal')
            plt.grid(True)
            ...
            for trajectory in all_trajectorys[1:]:
                ...
                p2.plot(x, y, marker='o', markersize=size, alpha=0.1, linewidth=2, color='blue')
            # walker trajectory
            for trajectory in walker_trajectorys:
                ...
                p2.plot(x, y, marker='o', markersize=size, alpha=0.3, linewidth=2, color='green')
            ...
            if len(image_front_queue)>1 and len(image_front_left_queue)>1 and len(image_front_right_queue)>1 and \
               ...
               len(lidar_queue)>1 and len(image_sematic_queue)>1 :
                ...
                img2 = np.hstack([lidar_image, image_back, image_sematic])
                # img3 = np.hstack([lidar_image, image_sematic ])
                img4 = np.vstack([img1, img2])
            ...
            # ====================================================================================
            time_end = time.time()
            print('vehile_num:', len(vehicles),  'observe_num', len(surrounding_vehicle), ' time cost:', round(time_end-time_start,3), 
