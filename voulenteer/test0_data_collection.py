...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            ego_control = world.player.get_control()
            # ==============================================================================================
            # limit speed
            ...
            current_scenario_info = args.scenario_config[world.scenario_idx]
            behavior_type = current_scenario_info['behavior_type']
            scenario_id  = str( current_scenario_info['scenario_id'] )
            # =================================== do something  =================================
            if cap.isOpened():
                success, driver_img = cap.read() 
                if not success:
                    print("There is no dirver image")
                    break

            if  len(image_front_queue)>1:
                image_front = image_front_queue[-1]
                # cv2.imshow("output", image_front)
                # cv2.imshow("output", driver_img)
                
                # check if saving path exist, if not, create file for current senarioID
                # tester_name 文件夹
                if not os.path.exists(args.tester_name):
                    os.mkdir(args.tester_name)
                # tester_name +  behavior_type文件夹
                behavior_types = ['lane_keep', 'left_turn', 'left_change', 'right_turn', 'right_change', 'free_style']
                if behavior_type not in behavior_types:
                    print('behavior_type 异常')
                    break
                else:
                    behavior_type_folder = os.path.join(args.tester_name, behavior_type)
                    if not os.path.exists(behavior_type_folder):
                        os.mkdir(behavior_type_folder)
                # tester_name + behavior_type + scenario_id命名的文件夹
                scenario_folder = os.path.join(behavior_type_folder, str(scenario_id))
                if not os.path.exists(scenario_folder):
                    os.mkdir(scenario_folder)
                # driver_image + scene_image + data json文件
                driver_image_folder = os.path.join(scenario_folder, 'driver_image')
                scene_image_folder = os.path.join(scenario_folder, 'scene_image')
                json_file_path = os.path.join(scenario_folder, 'driving_manipulation.json')
                if not os.path.exists(driver_image_folder):
                    os.mkdir(driver_image_folder)
                if not os.path.exists(scene_image_folder):
                    os.mkdir(scene_image_folder)
                if not os.path.exists(json_file_path):
                    with open(json_file_path, 'w') as json_file:
                        pass
                
                cur_time = time.time()
                saving_gap = cur_time - last_saving_time
                if world.enable_recording==True and saving_gap > args.saving_gap:
                    last_saving_time = time.time()
                    # time stamps
                    save_time = datetime.datetime.now()
                    save_time_str = save_time.strftime("%Y_%m_%d-%H_%M_%S.%f")
                    # driver image
                    driver_filename = save_time_str + ".jpg"
                    driver_file_path = os.path.join(driver_image_folder, driver_filename)
                    cv2.imwrite(driver_file_path, driver_img)
                    # scene image
                    scene_filename = save_time_str + ".jpg"
                    scene_file_path = os.path.join(scene_image_folder, scene_filename)
                    cv2.imwrite(scene_file_path, image_front)
                    # driving manipulation
                    driving_data = {
                        "timestamp": save_time_str,
                        "ego_x": ego_x,
                        "ego_y": ego_y,
                        "ego_z": ego_z,
                        "ego_yaw": ego_yaw,
                        "ego_vx": ego_velocity_x,
                        "ego_vy": ego_velocity_y,
                        "ego_vz": ego_velocity_z,
                        "ego_ax": ego_acc_x,
                        "ego_ay": ego_acc_y,
                        "ego_az": ego_acc_z,
                    ...
                    with open(json_file_path, 'a') as json_file:
                        json.dump(driving_data, json_file)
                        json_file.write('\n')
                    # ===================================================================================
