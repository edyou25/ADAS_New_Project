...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            if self.scenario_length == 0:
                spawn_point = carla.Transform(carla.Location(x=47.0, y=-117.0, z=5),
                                                carla.Rotation(pitch=-0.43, yaw=131.3, roll=-0.02))
            else:
                location_x = self.scenario_config[self.scenario_idx]['ego_start_point']['x']
                location_y = self.scenario_config[self.scenario_idx]['ego_start_point']['y']
                location_z = self.scenario_config[self.scenario_idx]['ego_start_point']['z']
...
def game_loop(args):
    ...
    try:
        ...
        # diy mot
        # spawn_point = carla.Transform(carla.Location(x=-274,y=111,z=5), carla.Rotation(pitch=-0.43, yaw=137.3,
        #                                                                               roll=-0.02))
        # blueprint = client.get_world().get_blueprint_library().filter("charger_2020")[0]
        # diy_mot = client.get_world().try_spawn_actor(blueprint, spawn_point)
        # diy_actor_list.append(diy_mot)
        ...
        while True:
            ...
            for vehicle in vehicles:
                if vehicle.id != ego_id:
                    other_tf = vehicle.get_transform()
